...
interface ParkSubmissionDashboardProps {
    onClose: () => void;
}

const ParkSubmissionDashboard = ({ onClose }: ParkSubmissionDashboardProps) => {
    ...
    const handleViewSubmission = (id: number | string) => {
    ...
    const createActionHandler = (
        actionFn: (id: number | string) => Promise<Record<string, unknown> | null>,
        actionName: string
    ) => {
        return async (id: number | string) => {
            try {
                ...
                console.error(`Failed to ${actionName} submission ${id}:`, err);
            }
        };
    };
    ...
    const selectedSubmission = useMemo(
        () => submissions.find((s) => s.id === viewSubmission),
                    ...
                    {!loading && !error && submissions.length > 0 && (
                        <ul className='space-y-4 sm:space-y-5'>
                            {submissions.map((submission) => {
                                ...
                                const { className: statusClass, label: statusLabel } = getStatusMeta(status ?? 'pending');
                                const equipmentList = Array.isArray(equipment) ? equipment : [];
                                const equipmentPreview = equipmentList.slice(0, 5).map((item) =>
                                    typeof item === 'string' ? item : (item?.name ?? 'Unknown')
                                );
                                const remainingEquipmentCount = equipmentList.length - equipmentPreview.length;
                                            ...
                                            <div className='flex flex-col gap-2'>
                                                <div className='flex flex-wrap items-center gap-3 text-xs uppercase tracking-wide text-slate-400'>
                                                    ...
                                                    <span className='text-slate-400'>
                                                        {equipmentList.length} equipment item
