...
- `src/components/parkSubmission/ParkSubmissionDashboard.jsx` and its nested moderation viewer
...
- `date`: ISO-8601 date format (YYYY-MM-DD). Frontend formats for display. If omitted, UI shows "Date TBA".
- `time`: 24-hour format (HH:mm) in UTC. Frontend converts to local timezone. If omitted, only date is shown.
- `distance`: Pre-formatted string (e.g., "0.8 mi away", "1.2 mi away"). Backend must format the distance value. Required if `lat`/`lng` query params were provided.
- `ctaLabel`: Optional. Defaults to `"View event"` when missing.
